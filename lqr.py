    ...
    for epoch in range(args.epoch):
        ...
        for i in range(args.m):
            ...
            for j in range(args.l):
                action = -np.dot(K + U_i, state)
                state, cost = env.step(action)
                C_i += cost
                sigma_i += np.dot(state, state.T)
                print(np.linalg.norm(action), np.linalg.norm(state))
        ...
        print(np.linalg.norm(K), np.linalg.norm(optimal_K), np.linalg.norm(K - optimal_K) / np.linalg.norm(optimal_K))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--l", default=100000000, type=int, help="roll-out length")
    parser.add_argument("--m", default=1, type=int, help="number of trajectories")
    parser.add_argument("--r", default=0.01, type=float, help="smoothing parameter")
    parser.add_argument("--epoch", default=1, type=int, help="number of training epochs")
